                ...
                for dest in vpc_dict:
                    ...
                    for index_1, dest_check in enumerate(vpc_dict[dest][0]):
                        ...
                        if source_ip_cidr.overlaps(dest_ip_cidr):
                            ...
                            print(
                                f"IP CIDRs overlap with the account {source}, {dest} for the range {source_ip_cidr}, {dest_ip_cidr} and {vpc_dict[source][1][index]}, {vpc_dict[dest][1][index_1]}")
    ...
    for acct_id in account_ids_list:
        ...
        try:
            resp = sts.assume_role(
                RoleArn=arn, RoleSessionName="Overlap-Cidr-info")
        except Exception as e:
            sys.stderr.write("assume role got exception/{}/{}/{}\n".format(type(e),
                                                                           repr(
                                                                               e),
                                                                           str(e)))
        vpc_cidrs, vpc_ids = get_vpc(acct_id, resp.get(
            'Credentials', {}) if resp else None)
